...
class WebInterface(object):
    ...
    def artistPage(self, ArtistID):
        myDB = db.DBConnection()
        artist = myDB.action(
            'SELECT * FROM artists WHERE ArtistID=?', [ArtistID]).fetchone()
        albums = myDB.select(
            'SELECT * from albums WHERE ArtistID=? order by ReleaseDate DESC', [ArtistID])
        ...
        extras_list = ["single", "ep", "compilation", "soundtrack", "live", "remix", "djmix",
            "mixtape_street", "spokenword", "audiobook", "broadcast", "interview", "other"]
        extras_dict = {}
    ...
    def albumPage(self, AlbumID):
        ...
        description = myDB.action(
            'SELECT * from descriptions WHERE ReleaseGroupID=?', [AlbumID]).fetchone()
    ...
    def addArtist(self, artistid):
        threading.Thread(
            target=importer.addArtisttoDB, args=[artistid]).start()
    ...
    def getExtras(self, ArtistID, newstyle=False, **kwargs):
        ...
        threading.Thread(
            target=importer.addArtisttoDB, args=[ArtistID, True, False]).start()
        raise cherrypy.HTTPRedirect("artistPage?ArtistID=%s" % ArtistID)
    ...
    def removeExtras(self, ArtistID, ArtistName):
        ...
        extraalbums = myDB.select(
            'SELECT AlbumID from albums WHERE ArtistID=? AND Status="Skipped" AND Type!="Album"', [ArtistID])
        for album in extraalbums:
            ...
            myDB.action('DELETE from alltracks WHERE ArtistID=? AND AlbumID=?', [
                        ArtistID, album['AlbumID']])
            myDB.action(
                'DELETE from releases WHERE ReleaseGroupID=?', [album['AlbumID']])
    ...
    def deleteArtist(self, ArtistID):
        ...
        namecheck = myDB.select(
            'SELECT ArtistName from artists where ArtistID=?', [ArtistID])
        for name in namecheck:
            artistname=name['ArtistName']
        myDB.action('DELETE from artists WHERE ArtistID=?', [ArtistID])

        rgids = myDB.select(
            'SELECT DISTINCT ReleaseGroupID FROM albums JOIN releases ON AlbumID = ReleaseGroupID WHERE ArtistID=?', [ArtistID])
        for rgid in rgids:
            myDB.action(
                'DELETE from releases WHERE ReleaseGroupID=?', [rgid['ReleaseGroupID']])

        myDB.action('DELETE from albums WHERE ArtistID=?', [ArtistID])
        myDB.action('DELETE from tracks WHERE ArtistID=?', [ArtistID])

        rgids = myDB.select(
            'SELECT DISTINCT ReleaseGroupID FROM allalbums JOIN releases ON AlbumID = ReleaseGroupID WHERE ArtistID=?', [ArtistID])
        for rgid in rgids:
            myDB.action(
                'DELETE from releases WHERE ReleaseGroupID=?', [rgid['ReleaseGroupID']])
        ...
        myDB.action(
            'UPDATE have SET Matched=NULL WHERE ArtistName=?', [artistname])
    ...
    def deleteEmptyArtists(self):
        ...
        for ArtistID in emptyArtistIDs:
            ...
            myDB.action('DELETE from alltracks WHERE ArtistID=?', [ArtistID])
            myDB.action(
                'INSERT OR REPLACE into blacklist VALUES (?)', [ArtistID])
    deleteEmptyArtists.exposed = True

    def refreshArtist(self, ArtistID):
        threading.Thread(
            target=importer.addArtisttoDB, args=[ArtistID, False, True]).start()
        raise cherrypy.HTTPRedirect("artistPage?ArtistID=%s" % ArtistID)
    ...
    def markAlbums(self, ArtistID=None, action=None, **args):
        ...
        for mbid in args:
            ...
            if ArtistID:
                ...
                ArtistIDT = myDB.action(
                    'SELECT ArtistID FROM albums WHERE AlbumID=?', [mbid]).fetchone()[0]
            myDB.action(
                'UPDATE artists SET TotalTracks=(SELECT COUNT(*) FROM tracks WHERE ArtistID = ? AND AlbumTitle IN (SELECT AlbumTitle FROM albums WHERE Status != "Ignored")) WHERE ArtistID = ?', [ArtistIDT, ArtistIDT])
        if ArtistID:
            raise cherrypy.HTTPRedirect("artistPage?ArtistID=%s" % ArtistID)
        else:
    ...
    def addArtists(self, action=None, **args):
        if action == "add":
            threading.Thread(
                target=importer.artistlist_to_mbids, args=[args, True]).start()
        if action == "ignore":
            myDB = db.DBConnection()
            for artist in args:
                myDB.action(
                    'DELETE FROM newartists WHERE ArtistName=?', [artist])
                myDB.action(
                    'UPDATE have SET Matched="Ignored" WHERE ArtistName=?', [artist])
                logger.info(
                    "Artist %s removed from new artist list and set to ignored" % artist)
        raise cherrypy.HTTPRedirect("home")
    ...
    def choose_specific_download(self, AlbumID):
        results = searcher.searchforalbum(
            AlbumID, choose_specific_download=True)
    ...
    def manageAlbums(self, Status=None):
        ...
        if Status == "Upcoming":
            ...
            albums = myDB.select(
                'SELECT * from albums WHERE Status=?', [Status])
        else:
    ...
    def manageUnmatched(self):
        ...
        for albums in have_albums:
            ...
            #     original_clean = None
                if original_clean == albums['CleanName']:
                    have_dict = {
                        'ArtistName' : albums['ArtistName'], 'AlbumTitle' : albums['AlbumTitle'] }
        ...
        check = set([(cleanName(d['ArtistName']).lower(), cleanName(
            d['AlbumTitle']).lower()) for d in headphones_album_dictionary])
        unmatchedalbums = [d for d in have_album_dictionary if (cleanName(
            d['ArtistName']).lower(), cleanName(d['AlbumTitle']).lower()) not in check]
    ...
    def markUnmatched(self, action=None, existing_artist=None, existing_album=None, new_artist=None, new_album=None):
        ...
        if action == "ignoreArtist":
            ...
            myDB.action(
                'UPDATE have SET Matched="Ignored" WHERE ArtistName=? AND Matched = "Failed"', [artist])

        elif action == "ignoreAlbum":
            artist = existing_artist
            album = existing_album
            myDB.action(
                'UPDATE have SET Matched="Ignored" WHERE ArtistName=? AND AlbumTitle=? AND Matched = "Failed"', (artist, album))
            ...
            if new_artist_clean != existing_artist_clean:
                ...
                for entry in have_tracks:
                    ...
                    if old_clean_filename.startswith(existing_artist_clean):
                        new_clean_filename = old_clean_filename.replace(
                            existing_artist_clean, new_artist_clean, 1)
                        ...
                        match_alltracks = myDB.action(
                            'SELECT CleanName from alltracks WHERE CleanName=?', [new_clean_filename]).fetchone()
                        if match_alltracks:
                            myDB.upsert(
                                "alltracks", newValueDict, controlValueDict)
                        match_tracks = myDB.action(
                            'SELECT CleanName, AlbumID from tracks WHERE CleanName=?', [new_clean_filename]).fetchone()
                        if match_tracks:
                            myDB.upsert(
                                "tracks", newValueDict, controlValueDict)
                            myDB.action(
                                'UPDATE have SET Matched="Manual" WHERE CleanName=?', [new_clean_filename])
                            update_count+=1
            ...
            existing_clean_string = existing_artist_clean+ \
                " "+existing_album_clean
            new_clean_string = new_artist_clean+" "+new_album_clean
            if existing_clean_string != new_clean_string:
                have_tracks = myDB.action(
                    'SELECT Matched, CleanName, Location, BitRate, Format FROM have WHERE ArtistName=? AND AlbumTitle=?', (existing_artist, existing_album))
                update_count = 0
                for entry in have_tracks:
                    old_clean_filename = entry['CleanName']
                    if old_clean_filename.startswith(existing_clean_string):
                        new_clean_filename = old_clean_filename.replace(
                            existing_clean_string, new_clean_string, 1)
                        myDB.action('UPDATE have SET CleanName=? WHERE ArtistName=? AND AlbumTitle=? AND CleanName=?', [
                                    new_clean_filename, existing_artist, existing_album, old_clean_filename])
                        controlValueDict = {"CleanName": new_clean_filename}
                        newValueDict = {"Location" : entry['Location'],
                                        "BitRate" : entry['BitRate'],
                                        "Format" : entry['Format']
                ...
                logger.info("Artist %s / Album %s already named appropriately; nothing to modify" %
        ...
        for albums in manualalbums:
            if albums['ArtistName'] and albums['AlbumTitle'] and albums['TrackTitle']:
                ...
                if albums['Matched'] == "Ignored" or albums['Matched'] == "Manual" or albums['CleanName'] != original_clean:
                    ...
                    manual_dict = { 'ArtistName' : albums['ArtistName'], 'AlbumTitle' : albums[
                        'AlbumTitle'], 'AlbumStatus' : album_status }
                    if manual_dict not in manual_albums:
                        manual_albums.append(manual_dict)
        manual_albums_sorted = sorted(
            manual_albums, key=itemgetter('ArtistName', 'AlbumTitle'))
    ...
    def markManual(self, action=None, existing_artist=None, existing_album=None):
        ...
        if action == "unignoreArtist":
            artist = existing_artist
            myDB.action(
                'UPDATE have SET Matched="Failed" WHERE ArtistName=? AND Matched="Ignored"', [artist])
            logger.info(
                "Artist: %s successfully restored to unmatched list" % artist)

        elif action == "unignoreAlbum":
            artist = existing_artist
            album = existing_album
            myDB.action(
                'UPDATE have SET Matched="Failed" WHERE ArtistName=? AND AlbumTitle=? AND Matched="Ignored"', (artist, album))
            logger.info(
                "Album: %s successfully restored to unmatched list" % album)

        elif action == "unmatchArtist":
            artist = existing_artist
            update_clean = myDB.select(
                'SELECT ArtistName, AlbumTitle, TrackTitle, CleanName, Matched from have WHERE ArtistName=?', [artist])
            update_count = 0
            for tracks in update_clean:
                original_clean = helpers.cleanName(
                    tracks['ArtistName']+" "+tracks['AlbumTitle']+" "+tracks['TrackTitle']).lower()
                album = tracks['AlbumTitle']
                track_title = tracks['TrackTitle']
                if tracks['CleanName'] != original_clean:
                    myDB.action('UPDATE tracks SET Location=?, BitRate=?, Format=? WHERE CleanName=?', [
                                None, None, None, tracks['CleanName']])
                    myDB.action('UPDATE alltracks SET Location=?, BitRate=?, Format=? WHERE CleanName=?', [
                                None, None, None, tracks['CleanName']])
                    myDB.action('UPDATE have SET CleanName=?, Matched="Failed" WHERE ArtistName=? AND AlbumTitle=? AND TrackTitle=?', (
            ...
            album = existing_album
            update_clean = myDB.select(
                'SELECT ArtistName, AlbumTitle, TrackTitle, CleanName, Matched from have WHERE ArtistName=? AND AlbumTitle=?', (artist, album))
            update_count = 0
            for tracks in update_clean:
                original_clean = helpers.cleanName(
                    tracks['ArtistName']+" "+tracks['AlbumTitle']+" "+tracks['TrackTitle']).lower()
                track_title = tracks['TrackTitle']
                if tracks['CleanName'] != original_clean:
                    album_id_check = myDB.action(
                        'SELECT AlbumID from tracks WHERE CleanName=?', [tracks['CleanName']]).fetchone()
                    if album_id_check:
    ...
    def markArtists(self, action=None, **args):
        ...
        for ArtistID in args:
            if action == 'delete':
                ...
                myDB.action(
                    'DELETE from allalbums WHERE ArtistID=?', [ArtistID])
                myDB.action(
                    'DELETE from alltracks WHERE ArtistID=?', [ArtistID])
                myDB.action(
                    'INSERT OR REPLACE into blacklist VALUES (?)', [ArtistID])
            elif action == 'pause':
                controlValueDict = {'ArtistID': ArtistID}
        ...
        if len(artistsToAdd) > 0:
            ...
            threading.Thread(
                target=importer.addArtistIDListToDB, args=[artistsToAdd]).start()
        raise cherrypy.HTTPRedirect("home")
    ...
    def importLastFMTag(self, tag, limit):
        threading.Thread(
            target=lastfm.getTagTopArtists, args=(tag, limit)).start()
        raise cherrypy.HTTPRedirect("home")
    ...
    def forcePostProcess(self, dir=None, album_dir=None):
        ...
        threading.Thread(target=postprocessor.forcePostProcess, kwargs={
                         'dir': dir, 'album_dir': album_dir}).start()
        raise cherrypy.HTTPRedirect("home")
    ...
    def getLog(self, iDisplayStart=0, iDisplayLength=100, iSortCol_0=0, sSortDir_0="desc", sSearch="", **kwargs):
        ...
        if sSearch == "":
            ...
            filtered = [row for row in headphones.LOG_LIST for column in row if sSearch.lower(
                ) in column.lower()]
        ...
        if iSortCol_0 == '1':
            ...
            sortcolumn = 1
        filtered.sort(
            key=lambda x: x[sortcolumn], reverse=sSortDir_0 == "desc")
    ...
    def getArtists_json(self, iDisplayStart=0, iDisplayLength=100, sSearch="", iSortCol_0='0', sSortDir_0='asc', **kwargs):
        ...
        if sSearch == "":
            ...
            query = 'SELECT * from artists WHERE ArtistSortName LIKE "%' + sSearch + '%" OR LatestAlbum LIKE "%' + \
                sSearch +'%"' + \
        ...
        if sortbyhavepercent:
            filtered.sort(key=lambda x: (float(x['HaveTracks'])/x['TotalTracks'] if x[
                          'TotalTracks'] > 0 else 0.0, x['HaveTracks'] if x['HaveTracks'] else 0.0), reverse=sSortDir_0 == "asc")

        # can't figure out how to change the datatables default sorting order when its using an ajax datasource so ill
        # just reverse it here and the first click on the "Latest Album" header
        # will sort by descending release date
    ...
    def getAlbumsByArtist_json(self, artist=None):
        ...
        album_list = myDB.select(
            "SELECT AlbumTitle from albums WHERE ArtistName=?", [artist])
    ...
    def clearhistory(self, type=None, date_added=None, title=None):
        ...
        if type:
            ...
            myDB.action(
                'DELETE from snatched WHERE Title=? AND DateAdded=?', [title, date_added])
    ...
    def generateAPI(self):
        ...
        apikey = hashlib.sha224(
            str(random.getrandbits(256)) ).hexdigest()[0:32]
    ...
    def forceScan(self, keepmatched=None):
        ...
        myDB.select(
            'UPDATE alltracks SET Location=NULL, BitRate=NULL, Format=NULL')
        myDB.select(
            'UPDATE tracks SET Location=NULL, BitRate=NULL, Format=NULL')
    ...
    def config(self):
        ...
        interface_list = [ name for name in os.listdir(
            interface_dir) if os.path.isdir(os.path.join(interface_dir, name)) ]
        ...
        extras_list = ["single", "ep", "compilation", "soundtrack", "live", "remix", "dj-mix",
            "mixtape/street", "spokenword", "audiobook", "broadcast", "interview", "other"]
        extras_dict = {}
    ...
    def configUpdate(self, http_host='0.0.0.0', http_username=None, http_port=8181, http_password=None, launch_browser=0, api_enabled=0, api_key=None,
        ...
        headphones.ENCODER_MULTICORE_COUNT = max(
            0, int(encoder_multicore_count))
        ...
        # Handle the variable config options. Note - keys with False values
        ...
        for kwarg in kwargs:
            if kwarg.startswith('newznab_host'):
                ...
                try:
                    newznab_enabled = int(
                        kwargs['newznab_enabled' + newznab_number])
                except KeyError:
                    newznab_enabled = 0
    ...
    def addReleaseById(self, rid, rgid=None):
        threading.Thread(
            target=importer.addReleaseById, args=[rid, rgid]).start()
        if rgid:
