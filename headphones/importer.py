...
blacklisted_special_artist_names = [
...
def is_exists(artistid):
    ...
    artistlist = myDB.select(
        'SELECT ArtistID, ArtistName from artists WHERE ArtistID=?', [artistid])
...
def artistlist_to_mbids(artistlist, forced=False):
    ...
    for artist in artistlist:
        ...
        try:
            ...
            logger.info(
                'MusicBrainz query turned up no matches for: %s' % artist)
            continue
        ...
        if not forced:
            bl_artist = myDB.action(
                'SELECT * FROM blacklist WHERE ArtistID=?', [artistid]).fetchone()
            if bl_artist or artistid in blacklisted_special_artists:
                logger.info("Artist ID for '%s' is either blacklisted or Various Artists. To add artist, you must do it manually (Artist ID: %s)" % (
                    artist, artistid))
                continue
        ...
        # Just update the tracks if it does
            ...
            havetracks = len(myDB.select('SELECT TrackTitle from tracks WHERE ArtistID=?', [
                             artistid])) + len(myDB.select('SELECT TrackTitle from have WHERE ArtistName like ?', [artist]))
            myDB.action(
                'UPDATE artists SET HaveTracks=? WHERE ArtistID=?', [havetracks, artistid])
...
def addArtisttoDB(artistid, extrasonly=False, forcefull=False):
    ...
    # Putting this here to get around the circular import. We're using this to
    # update thumbnails for artist/albums
    from headphones import cache

    # Can't add various artists - throws an error from MB
    if artistid in blacklisted_special_artists:
        logger.warn(
            'Cannot import blocked special purpose artist with id' + artistid)
        return
    ...
    # Only modify the Include Extras stuff if it's a new artist. We need it
    # early so we know what to fetch
    if not dbartist:
    ...
    if artist and artist.get('artist_name') in blacklisted_special_artist_names:
        logger.warn(
            'Cannot import blocked special purpose artist: %s' % artist.get('artist_name'))
    ...
    try:
        db_artist = myDB.action(
            'SELECT IncludeExtras, Extras from artists WHERE ArtistID=?', [artistid]).fetchone()
        includeExtras = db_artist['IncludeExtras']
    except IndexError:
        includeExtras = False

    # Clean all references to release group in dB that are no longer
    # referenced in musicbrainz
    group_list = []
    force_repackage = 0
    ...
    if len(artist['releasegroups']) != 0 and not extrasonly:
        ...
        remove_missing_groups_from_albums = myDB.action(
        ...
        for items in remove_missing_groups_from_albums:
            if items['AlbumID'] not in group_list:
                ...
                myDB.action(
                    "DELETE FROM albums WHERE AlbumID=?", [items['AlbumID']])
                myDB.action(
                    "DELETE FROM allalbums WHERE AlbumID=?", [items['AlbumID']])
                myDB.action(
                    "DELETE FROM tracks WHERE AlbumID=?", [items['AlbumID']])
                myDB.action(
                    "DELETE FROM alltracks WHERE AlbumID=?", [items['AlbumID']])
                myDB.action(
                    'DELETE from releases WHERE ReleaseGroupID=?', [items['AlbumID']])
                logger.info("[%s] Removing all references to release group %s to reflect MusicBrainz" % (
                    artist['artist_name'], items['AlbumID']))
                force_repackage = 1
    elif extrasonly:
        # Not really sure what we're doing here but don't want to log the
        # message below if we're fetching extras only
        pass
    else:
        logger.info("[%s] There was either an error pulling data from MusicBrainz or there might not be any releases for this category" % artist[
                    'artist_name'])

    # Then search for releases within releasegroups, if releases don't exist,
    # then remove from allalbums/alltracks
    album_searches = []
    ...
    for rg in artist['releasegroups']:
        ...
        rgid = rg['id']
        skip_log = 0
        # Make a user configurable variable to skip update of albums with
        # release dates older than this date (in days)
        pause_delta = headphones.MB_IGNORE_AGE

        rg_exists = myDB.action(
            "SELECT * from albums WHERE AlbumID=?", [rg['id']]).fetchone()
        ...
        if new_releases != 0:
            ...
            myDB.action(
                'DELETE from releases WHERE ReleaseGroupID=?', [rg['id']])
            ...
            for items in find_hybrid_releases:
                # don't include hybrid information, since that's what we're
                # replacing
                if items['ReleaseID'] != rg['id']:
                    ...
                    find_hybrid_tracks = myDB.action(
                        "SELECT * from alltracks WHERE ReleaseID=?", [hybrid_release_id])
                    totalTracks = 1
                    hybrid_track_array = []
            ...
            for track in hybridrelease['Tracks']:
                ...
                match = myDB.action(
                    'SELECT Location, BitRate, Format from have WHERE CleanName=?', [cleanname]).fetchone()

                if not match:
                    match = myDB.action('SELECT Location, BitRate, Format from have WHERE ArtistName LIKE ? AND AlbumTitle LIKE ? AND TrackTitle LIKE ?', [
                ...
                if match:
                    ...
                    myDB.action(
                        'UPDATE have SET Matched=? WHERE Location=?', (rg['id'], match['Location']))
            ...
            have_track_count = len(myDB.select(
                'SELECT * from tracks WHERE AlbumID=? AND Location IS NOT NULL', [rg['id']]))
            ...
            if rg_exists:
                if rg_exists['Status'] == 'Skipped' and ((have_track_count/float(total_track_count)) >= (headphones.ALBUM_COMPLETION_PCT/100.0)):
                    myDB.action(
                        'UPDATE albums SET Status=? WHERE AlbumID=?', ['Downloaded', rg['id']])
    ...
    if errors:
        logger.info("[%s] Finished updating artist: %s but with errors, so not marking it as updated in the database" % (
            artist['artist_name'], artist['artist_name']))
    else:
        myDB.action(
            'DELETE FROM newartists WHERE ArtistName = ?', [artist['artist_name']])
...
def finalize_update(artistid, artistname, errors=False):
    # Moving this little bit to it's own function so we can update have tracks
    # & latest album when deleting extras

    myDB = db.DBConnection()

    latestalbum = myDB.action(
        'SELECT AlbumTitle, ReleaseDate, AlbumID from albums WHERE ArtistID=? order by ReleaseDate DESC', [artistid]).fetchone()
    totaltracks = len(myDB.select(
        'SELECT TrackTitle from tracks WHERE ArtistID=? AND AlbumID IN (SELECT AlbumID FROM albums WHERE Status != "Ignored")', [artistid]))
    ...
    havetracks = len(myDB.select('SELECT TrackTitle from tracks WHERE ArtistID=? AND Location IS NOT NULL', [
                     artistid])) + len(myDB.select('SELECT TrackTitle from have WHERE ArtistName like ? AND Matched = "Failed"', [artistname]))
...
def addReleaseById(rid, rgid=None):
    ...
    release_dict = None
    results = myDB.select(
        "SELECT albums.ArtistID, releases.ReleaseGroupID from releases, albums WHERE releases.ReleaseID=? and releases.ReleaseGroupID=albums.AlbumID LIMIT 1", [rid])
    ...
    if not rgid:
        ...
        logger.debug("Didn't find releaseID " + rid + \
                     " in the cache. Looking up its ReleaseGroupID")
        try:
            release_dict = mb.getRelease(rid)
        except Exception, e:
            logger.info(
                'Unable to get release information for Release %s: %s', rid, e)
            if status == 'Loading':
                myDB.action("DELETE FROM albums WHERE AlbumID=?", [rgid])
            return
        if not release_dict:
            logger.info(
                'Unable to get release information for Release %s: no dict', rid)
            if status == 'Loading':
                myDB.action("DELETE FROM albums WHERE AlbumID=?", [rgid])
            return
    ...
    # we don't want to make more calls to MB here unless we have to, could be
    # happening quite a lot
    rg_exists = myDB.select("SELECT * from albums WHERE AlbumID=?", [rgid])

    # make sure the artist exists since I don't know what happens later if it
    # doesn't
    ...
    if not artist_exists and release_dict:
        ...
        logger.info(u"Now manually adding: " + \
                    release_dict['artist_name'] + " - with status Paused")
    ...
    #it should never be the case that we have an rg and not the artist
    if not rg_exists and release_dict or status == 'Loading' and release_dict:
                                                                                # but
                                                                                # if
                                                                                # it
                                                                                # is
                                                                                ...
                                                                                # fail
        logger.info(
            u"Now adding-by-id album (" + release_dict['title'] + ") from id: " + rgid)
        controlValueDict = {"AlbumID":  rgid}
        if status != 'Loading':
            status = 'Wanted'
        ...
        # keep a local cache of these so that external programs that are adding
        # releasesByID don't hammer MB
        myDB.action(
            'INSERT INTO releases VALUES( ?, ?)', [rid, release_dict['rgid']])
        ...
        for track in release_dict['tracks']:
            ...
            if not match:
                match = myDB.action('SELECT Location, BitRate, Format from have WHERE ArtistName LIKE ? AND AlbumTitle LIKE ? AND TrackTitle LIKE ?', [
                                    release_dict['artist_name'], release_dict['rg_title'], track['title']]).fetchone()
    ...
    if len(tracks) > 0:
        ...
        for track in tracks:
            try:
                ...
                logger.info(
                    "Exception from MediaFile for: " + track['Location'] + " : " + str(e))
                continue
            controlValueDict = {"TrackID":  track['TrackID']}
            newValueDict = {"Format": f.format}
            myDB.upsert("tracks", newValueDict, controlValueDict)
        logger.info('Finished finding media format for %s files' % len(tracks))
    havetracks = myDB.select(
        'SELECT * from have WHERE Location IS NOT NULL and Format IS NULL')
    if len(havetracks) > 0:
        ...
        logger.info('Finished finding media format for %s files' %
                    len(havetracks))
...
def getHybridRelease(fullreleaselist):
    ...
    if len(fullreleaselist) == 0:
        raise Exception(
            "getHybridRelease was called with an empty fullreleaselist")
    ...
    # necessary to make dates that miss the month and/or day show up after
    # full dates
    def getSortableReleaseDate(releaseDate):
        if releaseDate == None:
            # change this value to change the sorting behaviour of none,
            # returning 'None' will put it at the top
            return 'None';
    ...
    sortable_release_list.sort(
        key=lambda x: getSortableReleaseDate(x['releasedate']))

    average_tracks = sum(
        x['trackscount'] for x in sortable_release_list) / float(len(sortable_release_list))
    for item in sortable_release_list:
        item['trackscount_delta'] = abs(average_tracks - item['trackscount'])

    a = helpers.multikeysort(
        sortable_release_list, ['-hasasin', 'country', 'format', 'trackscount_delta'])
