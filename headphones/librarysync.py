...
# specifying append=True, ArtistID & ArtistName
def libraryScan(dir=None, append=False, ArtistID=None, ArtistName=None, cron=False):
    ...
    if not append:
        ...
        tracks = myDB.select(
            'SELECT Location, TrackID from alltracks WHERE Location IS NOT NULL')

        for track in tracks:
            encoded_track_string = track[
                'Location'].encode(headphones.SYS_ENCODING)
        ...
        for track in del_have_tracks:
            ...
            if not os.path.isfile(encoded_track_string):
                if track['ArtistName']:
                    # Make sure deleted files get accounted for when updating
                    # artist track counts
                    new_artists.append(track['ArtistName'])
                myDB.action(
                    'DELETE FROM have WHERE Location=?', [track['Location']])
                logger.info('File %s removed from Headphones, as it is no longer on disk' %
                            encoded_track_string.decode(headphones.SYS_ENCODING, 'replace'))
    ...
    for r, d, f in os.walk(dir):
        ...
        # using a list comprehension will not work correctly for nested
        # subdirectories (os.walk keeps its original list)
        for directory in d[:]:
        ...
        for files in f:
            ...
            if any(files.lower().endswith('.' + x.lower()) for x in headphones.MEDIA_FORMATS):
                ...
                if file_count == 0 and r.replace(dir, '') !='':
                    logger.info("[%s] Now scanning subdirectory %s" % (dir.decode(
                        headphones.SYS_ENCODING, 'replace'), subdirectory.decode(headphones.SYS_ENCODING, 'replace')))
                ...
                # We need the unicode path to use for logging, inserting into
                # database
                unicode_song_path = song.decode(
                    headphones.SYS_ENCODING, 'replace')
                ...
                # TODO: skip adding songs without the minimum requisite
                # information (just a matter of putting together the right if
                # statements)
                ...
                check_exist_song = myDB.action(
                    "SELECT * FROM have WHERE Location=?", [unicode_song_path]).fetchone()
                # Only attempt to match songs that are new, haven't yet been
                # matched, or metadata has changed.
                if not check_exist_song:
                    ...
                    if check_exist_song['ArtistName'] != f_artist or check_exist_song['AlbumTitle'] != f.album or check_exist_song['TrackTitle'] != f.title:
                        # Important track metadata has been modified, need to
                        # run matcher again
    ...
    logger.info(
        "%s new/modified songs found and added to the database" % new_song_count)
    song_list = myDB.action("SELECT * FROM have WHERE Matched IS NULL AND LOCATION LIKE ?", [
                            dir.decode(headphones.SYS_ENCODING, 'replace')+"%"])
    total_number_of_songs = myDB.action("SELECT COUNT(*) FROM have WHERE Matched IS NULL AND LOCATION LIKE ?", [
    ...
    # When we insert into the database, the tracks with the most specific
    # information will overwrite the more general matches

    ##############song_list = helpers.multikeysort(song_list, ['ReleaseID', 'TrackID'])
    song_list = helpers.multikeysort(song_list, ['ArtistName', 'AlbumTitle'])

    # We'll use this to give a % completion, just because the track matching
    # might take a while
    song_count = 0
    latest_artist = []
    ...
    for song in song_list:
        ...
        # print song['ArtistName']+' - '+song['AlbumTitle']+' -
        # '+song['TrackTitle']
        song_count += 1
        ...
        if completion_percentage%10 == 0:
            logger.info(
                "Track matching is " + str(completion_percentage) + "% complete")
        ...
        if song['ArtistName'] and song['AlbumTitle'] and song['TrackTitle']:
            ...
            track = myDB.action('SELECT ArtistName, AlbumTitle, TrackTitle, AlbumID from tracks WHERE ArtistName LIKE ? AND AlbumTitle LIKE ? AND TrackTitle LIKE ?', [
            ...
            alltrack = myDB.action('SELECT ArtistName, AlbumTitle, TrackTitle, AlbumID from alltracks WHERE ArtistName LIKE ? AND AlbumTitle LIKE ? AND TrackTitle LIKE ?', [
                                   song['ArtistName'], song['AlbumTitle'], song['TrackTitle']]).fetchone()
            if alltrack:
                ...
                alltrack = myDB.action('SELECT CleanName, AlbumID from alltracks WHERE CleanName LIKE ?', [
                                       song['CleanName']]).fetchone()
                if alltrack:
    ...
    if not append:
        ...
        # There was a bug where artists with special characters (-,') would
        # show up in new artists.
        artist_list = [f for f in unique_artists if helpers.cleanName(
            f).lower() not in [helpers.cleanName(x[0]).lower() for x in current_artists]]
        artists_checked = [f for f in unique_artists if helpers.cleanName(
            f).lower() in [helpers.cleanName(x[0]).lower() for x in current_artists]]
        ...
        for artist in artists_checked:
            ...
            havetracks = len(myDB.select('SELECT TrackTitle from tracks WHERE ArtistName like ? AND Location IS NOT NULL', [
                             artist])) + len(myDB.select('SELECT TrackTitle from have WHERE ArtistName like ? AND Matched = "Failed"', [artist]))
            ...
            myDB.action(
                'UPDATE artists SET HaveTracks=? WHERE ArtistName=?', [havetracks, artist])
        ...
        # If we're appending a new album to the database, update the artists
        # total track counts
...
# ADDED THIS SECTION TO MARK ALBUMS AS DOWNLOADED IF ARTISTS ARE ADDED EN
# MASSE BEFORE LIBRARY IS SCANNED
def update_album_status(AlbumID=None):
    ...
    for album in album_status_updater:
        ...
        if total_tracks != 0:
            ...
            album_completion = 0
            logger.info(
                'Album %s does not have any tracks in database' % album['AlbumTitle'])
        ...
        myDB.upsert(
            "albums", {'Status'   : new_album_status}, {'AlbumID'   : album['AlbumID']})
